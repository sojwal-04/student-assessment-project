...
const authMiddleware = {
  ...
  isFaculty: asyncHandler(async (req, res, next) => {
    await verifyToken(req, res, async () => {
  ...
  isAdminOrFaculty: asyncHandler(async (req, res, next) => {
    await verifyToken(req, res, async () => {
      if (
        req.user &&
      ...
      return res.status(403).json({
        success: false,
        message: "Access denied. Only Admins or Faculty are allowed.",
      });
    });
  }),
  ...
  isUserOrBloodBank: asyncHandler(async (req, res, next) => {
    await verifyToken(req, res, async () => {
      if (
        req.user &&
        (req.user.role === "user" || req.user.role === "bloodBank")
      ) {
        return next();
      }
      return res.status(403).json({
